...
void StructWrite::OpenFile(const std::string &file_name, TFile *&m_file, TTree *&m_tree)
{
    ...
    m_tree->Branch("evt.matrix", "TMatrixDSparse", &evt.matrix);
...
int main(int argc, char *argv[]) {
    ...
    for (Long64_t i = 0; i < nentries; i++) {
        ...
        // Only fill non-zero elements in the sparse matrix
        ...
        if (matrix) {
            ...
            for (int row = 0; row < matrix->GetNrows(); row++) {
                for (int col = 0; col < matrix->GetNcols(); col++) {
                    ...
                    if (value != 0) {
                        writer.evt.matrix->operator()(row, col) = value;

                        //  // Debugging output
                        // std::cout << "Non-zero element added: "
                        //         << "Row: " << row << ", Col: " << col 
            ...
            // std::cout << "Total non-zero elements added to sparse matrix: " 
